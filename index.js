...
function addGamesToPage(games) {
    ...
    for (let game of games) {
        ...
        cardDisplay.classList.add("game-card")
        ...
        gamesContainer.appendChild(cardDisplay);
    }
}
...
function filterUnfundedOnly() {
    ...
    const unmet = GAMES_JSON.filter((game) => {
...
function filterFundedOnly() {
    ...
    const met = GAMES_JSON.filter((game) => {
        return game.pledged >= game.goal;
    });
...
// show only games that are searched
function filterSearchedOnly(searchTerm) {
    deleteChildElements(gamesContainer);
...
searchBar.addEventListener("input", (event) => {
    filterSearchedOnly(event.target.value);
});
/*************************************************************************************
...
let displayStr = `A total of $${amount.toLocaleString("en-US")} has been raised for ${GAMES_JSON.length - unfundedCount} games. Currently, ${unfundedCount} ${unfundedCount <= 1 ? "game remains" : "games remain"} unfunded. We need your help to fund these amazing games!`

// create a new DOM element containing the template string and append it to the description container
const unfundedStatement = document.createElement("p");

unfundedStatement.innerHTML = displayStr;
descriptionContainer.appendChild(unfundedStatement);
/************************************************************************************
